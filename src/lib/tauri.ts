...
export type SkillScope = "personal" | "org" | "project";
...
export interface ManagedSkill {
  ...
  tags: string[];
  scope: SkillScope;
  server_skill_id: string | null;
  content_hash?: string | null;
  category?: string | null;
  /** Human-readable semver from SKILL.md; falls back to content_hash prefix in UI. */
...
export interface InstallFromServerResult {
  skill_id: string;
  skill_name: string;
}

export const installFromServer = (
  baseUrl: string,
  token: string,
  serverSkillId: string
) =>
  invoke<InstallFromServerResult>("install_from_server", {
    baseUrl,
    token,
    serverSkillId,
  });

export const checkServerSkillUpdate = (
  baseUrl: string,
  token: string,
  skillId: string,
  force?: boolean
) =>
  invoke<ManagedSkill>("check_server_skill_update", {
    baseUrl,
    token,
    skillId,
    force: force ?? false,
...
export const updateSkillFromServer = (
  baseUrl: string,
  token: string,
  skillId: string
) =>
  invoke<UpdateSkillResult>("update_skill_from_server", {
    baseUrl,
    token,
    skillId,
  });
...
export const setSkillScope = (skillId: string, scope: SkillScope) =>
  invoke<ManagedSkill>("set_skill_scope", { skillId, scope });

export const setSkillCategory = (skillId: string, category: string | null) =>
  invoke<ManagedSkill>("set_skill_category", { skillId, category });

export const setServerSkillId = (skillId: string, serverSkillId: string | null) =>
  invoke<void>("set_server_skill_id", { skillId, serverSkillId });

export const getLinkedServerProjectId = () =>
  invoke<string | null>("get_linked_server_project_id");

export const setProjectServerLink = (projectId: string, serverProjectId: string | null) =>
  invoke<Project>("set_project_server_link", { projectId, serverProjectId });
...
export const projectAgentHasSkillDir = (
  projectId: string,
  agent: string,
  dirName: string
) =>
  invoke<boolean>("project_agent_has_skill_dir", { projectId, agent, dirName });
