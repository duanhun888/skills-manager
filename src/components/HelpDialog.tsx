import { BookOpen, FolderTree, Globe, Layers3, Map, Network, RefreshCw, Settings2, Sparkles, X } from "lucide-react";
import { useTranslation } from "react-i18next";
import { useApp } from "../context/AppContext";
import { AppModal } from "./AppModal";

const GUIDE_ICONS = [Map, Network, Layers3, BookOpen, Sparkles, Globe, FolderTree, RefreshCw, Settings2];

const SKILL_LEVEL_KEYS = ["personal", "org", "project"] as const;
    ...
    >
      <div className="relative w-full max-w-[640px] overflow-hidden rounded-[28px] border border-border bg-bg-secondary shadow-[0_40px_90px_rgba(0,0,0,0.45)]">
        ...
        <div className="max-h-[min(72vh,720px)] space-y-3 overflow-y-auto px-5 py-5">
          {(["workflows", "skillLevels", "presets", "install", "sync", "global", "projects", "backup", "settings"] as const).map((key, index) => {
            ...
            return (
              ...
              >
                ...
                <div>
                  ...
                  {key === "skillLevels" ? (
                    <ul className="mt-2 list-disc space-y-1.5 pl-4 text-[13px] leading-5 text-muted">
                      {SKILL_LEVEL_KEYS.map((level) => (
                        <li key={level}>{t(`help.items.skillLevels.examples.${level}`)}</li>
                      ))}
                    </ul>
                  ) : null}
                  {key === "skillLevels" ? (
                    <p className="mt-2 text-[13px] leading-5 text-secondary">{t("help.items.skillLevels.hint")}</p>
                  ) : null}
                </div>
              </div>
            );
