...
interface RowData {
  ...
  orphanDirByAgent: Record<string, boolean>;
...
export function SkillProjectsSection({ skill, projects, onChanged }: Props) {
      ...
      const results = await Promise.all(
        projects.map(async (p) => {
          try {
            ...
            const orphanDirByAgent: Record<string, boolean> = {};
            if (dirName) {
              await Promise.all(
                targets
                  .filter((t) => t.installed && t.enabled)
                  .map(async (target) => {
                    const exists = await api.projectAgentHasSkillDir(
                      p.id,
                      target.key,
                      dirName
                    );
                    orphanDirByAgent[target.key] = exists;
                  })
              );
            }
            return [p.id, {
  ...
  const getAgentState = (row: RowData | undefined, target: ProjectAgentTarget) => {
    ...
    if (row.dirName && row.orphanDirByAgent[target.key]) {
      return "conflict";
    }
    return "available";
  };
