...
import { fetchMyProjects, getStoredToken, type ServerProject, createServerProject } from "../lib/serverApi";
...
export function Settings() {
  ...
  const { tools, presets, refreshTools, openHelp, projects, refreshProjects } = useApp();
  const weakPresetNames = useMemo(() => findWeakPresetNames(presets), [presets]);
  ...
  const [serverApiUrlInput, setServerApiUrlInput] = useState("");
  const [serverApiUrlSaving, setServerApiUrlSaving] = useState(false);
  ...
  useEffect(() => {
    setServerApiUrlInput(serverApiUrl);
  }, [serverApiUrl]);

  useEffect(() => {
    if (!isServerMode || !isAuthenticated || !serverApiUrl) {
      setServerProjects([]);
      return;
    }
  ...
  }, [isServerMode, isAuthenticated, serverApiUrl]);

  const handleProjectServerLink = async (
    localProjectId: string,
    serverProjectId: string
  ) => {
    setProjectLinkSaving(localProjectId);
    try {
      await api.setProjectServerLink(
        localProjectId,
        serverProjectId || null
      );
      await refreshProjects();
      toast.success(t("settings.projectServerLinkSaved"));
    } catch (e) {
      toast.error(getErrorMessage(e, t("common.error")));
    } finally {
      setProjectLinkSaving(null);
    }
  };

  const handleCreateServerProject = async (
    localProjectId: string,
    localProjectName: string
  ) => {
    if (!serverApiUrl || !user) return;
    const token = getStoredToken();
    if (!token) return;
    setCreatingServerProject(true);
    try {
      const existing = serverProjects.find(
        (p) => p.name.toLowerCase() === localProjectName.toLowerCase()
      );
      if (existing) {
        await api.setProjectServerLink(localProjectId, existing.id);
        await refreshProjects();
        toast.success(t("settings.projectServerLinkExists", { name: existing.name }));
        return;
      }
      const created = await createServerProject(
        serverApiUrl,
        token,
        localProjectName,
        user.id
      );
      const list = await fetchMyProjects(serverApiUrl, token);
      setServerProjects(list);
      await api.setProjectServerLink(localProjectId, created.id);
      await refreshProjects();
      toast.success(t("settings.projectServerLinkCreated", { name: created.name }));
    } catch (e) {
      toast.error(getErrorMessage(e, t("common.error")));
    } finally {
      setCreatingServerProject(false);
    }
  };
  ...
  const handleReportIssue = async () => {
    ...
    try {
      ...
      const parts = [
        "**Diagnostics** (auto-collected by 芯宏Skills仓库)",
        "",
  ...
  const handleSaveServerApiUrl = async () => {
    setServerApiUrlSaving(true);
    try {
      const trimmed = serverApiUrlInput.trim();
      await setServerApiUrl(trimmed);
      toast.success(t("settings.serverApiUrlSaved"));
      ...
      <div className="space-y-6">
        ...
        <section>
          ...
          <div className="app-panel overflow-hidden divide-y divide-border-subtle">
            ...
            {/* Default preset */}
            {weakPresetNames.length > 0 ? (
              <div className="px-4 py-3">
                <StatusBanner
                  compact
                  title={t("settings.weakPresetNamesTitle")}
                  description={t("settings.weakPresetNamesDesc", {
                    names: weakPresetNames.join("、"),
                  })}
                  tone="warning"
                />
              </div>
        ...
        <section>
          ...
          <div className="app-panel overflow-hidden divide-y divide-border-subtle">
            <div className="px-4 py-3">
              ...
              <p className="text-[13px] text-muted mb-1.5">{t("settings.proxyUrlDesc")}</p>
              <ul className="text-[12px] text-muted mb-2 list-none space-y-1 pl-0">
                <li className="flex gap-1.5">
                  <span className="text-emerald-600 dark:text-emerald-400 shrink-0">✓</span>
                  <span>{t("settings.proxyUrlApplies")}</span>
                </li>
                <li className="flex gap-1.5">
                  <span className="text-muted shrink-0">—</span>
                  <span>{t("settings.proxyUrlNotApplies")}</span>
                </li>
              </ul>
              <div className="flex flex-wrap items-center gap-2">
                <input
        ...
        {/* Personal Git backup (optional — not org central repo) */}
        <section>
          <h2 className="app-section-title mb-3">
        ...
        {/* Central server account */}
        <section>
          <h2 className="app-section-title mb-3">{t("settings.serverAccount")}</h2>
          <div className="app-panel overflow-hidden divide-y divide-border-subtle">
            ...
            <div className="px-4 py-3">
              ...
              <p className="text-[13px] text-muted mb-2">{t("settings.serverApiUrlDesc")}</p>
              <div className="flex flex-wrap items-center gap-2">
                <input
                  type="text"
                  value={serverApiUrlInput}
                  onChange={(e) => setServerApiUrlInput(e.target.value)}
                  placeholder={t("settings.serverApiUrlPlaceholder")}
                  className={`${fieldClass} min-w-0 flex-1 font-mono`}
                />
                <button
                  onClick={handleSaveServerApiUrl}
                  disabled={serverApiUrlSaving}
                  className={`${actionButtonClass} bg-surface-hover hover:bg-surface-active text-tertiary border-border`}
                >
                  {serverApiUrlSaving ? (
                    <Loader2 className="w-3 h-3 animate-spin" />
              ...
              {isServerMode && !isAuthenticated && (
                <p className="text-[12px] text-amber-600 dark:text-amber-400 mt-2">
                  {t("settings.serverLoginRequired")}
                </p>
              )}
            </div>
            )}
            {SERVER_API_URL_FIXED && isServerMode && !isAuthenticated && (
            <div className="px-4 py-3">
              <p className="text-[13px] text-muted">{t("settings.serverCentralDesc")}</p>
              <p className="text-[12px] text-amber-600 dark:text-amber-400 mt-2">
                {t("settings.serverLoginRequired")}
              </p>
            </div>
            ...
            {isAuthenticated && user && (
              <div className="px-4 py-3 flex flex-wrap items-center justify-between gap-3">
                <div>
                  <h3 className="text-[13px] text-secondary font-medium">{t("settings.signedInAs")}</h3>
                  <p className="text-[13px] text-primary">
                    {user.display_name || user.username}
                    <span className="text-muted ml-2 font-mono text-[12px]">@{user.username}</span>
                  </p>
                  {user.roles && (
                    <p className="text-[12px] text-muted mt-0.5">{user.roles}</p>
                  )}
                </div>
                <button
                  type="button"
            ...
            {isAuthenticated && projects.length > 0 && (
              <div className="px-4 py-3">
                <h3 className="text-[13px] text-secondary font-medium mb-0.5">
                  {t("settings.projectServerLink")}
                </h3>
                <p className="text-[13px] text-muted mb-3">{t("settings.projectServerLinkDesc")}</p>
                {serverProjects.length === 0 && projects[0] && (
                  <div className="mb-3 rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-[12px] text-amber-700 dark:text-amber-300">
                    <p className="mb-2">{t("settings.projectServerLinkEmpty")}</p>
                    <button
                      type="button"
                      disabled={creatingServerProject}
                      onClick={() =>
                        handleCreateServerProject(projects[0].id, projects[0].name)
                      }
                      className={`${actionButtonClass} bg-surface-hover hover:bg-surface-active text-tertiary border-border`}
                    >
                      {creatingServerProject ? (
                        <Loader2 className="w-3 h-3 animate-spin" />
                      ) : null}
                      {t("settings.projectServerLinkCreate", { name: projects[0].name })}
                    </button>
                  </div>
                )}
                <div className="space-y-2">
                  {projects.map((project) => (
                    ...
                    >
                      <span className="text-primary min-w-[120px] truncate" title={project.path}>
                        {project.name}
                      </span>
                      <select
                        className={`${fieldClass} min-w-[180px] flex-1`}
                        value={project.server_project_id ?? ""}
                        disabled={projectLinkSaving === project.id}
                        onChange={(e) => handleProjectServerLink(project.id, e.target.value)}
                      >
                        <option value="">{t("settings.projectServerLinkNone")}</option>
                        {serverProjects.map((sp) => (
                          <option key={sp.id} value={sp.id}>
                            {sp.name}
                            {serverProjects.filter((p) => p.name === sp.name).length > 1
                              ? ` (${sp.id.slice(0, 8)})`
                              : ""}
                          </option>
                        ))}
                      </select>
                      {projectLinkSaving === project.id && (
