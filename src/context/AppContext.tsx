...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const openSkillDetailById = useCallback((skillId: string) => {
  ...
  const weakPresetToastShownRef = useRef(false);
  ...
  useEffect(() => {
    if (loading || weakPresetToastShownRef.current) return;
    const weak = findWeakPresetNames(presets);
    if (weak.length === 0) return;
    weakPresetToastShownRef.current = true;
    toast.info(i18n.t("preset.weakNameToast", { names: weak.join("、") }), {
      duration: 10_000,
    });
  }, [loading, presets]);
