...
import { sanitizeAttachmentDataUrl } from "@/util/media"
  ...
  Effect.gen(function* () {
    ...
    const createUserMessage = Effect.fn("SessionPrompt.createUserMessage")(function* (input: PromptInput) {
      ...
      const parts = yield* Effect.forEach(resolvedParts, (part) => {
        ...
        if (!sanitized.mime.startsWith("image/")) return Effect.succeed(sanitized)
        return image.normalize(sanitized).pipe(
          Effect.catchIf(
