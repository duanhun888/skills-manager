  ...
  return {
    ...
    async openAttachmentPickerDialog(opts, onFile) {
      ...
      try {
        for (const file of result.files) {
          const buffer = await window.api.readPickedFile(result.token, file.path)
          const type = mimeFromFilename(file.name)
          const selected = new File([buffer], file.name, type ? { type } : undefined)
    ...
    fetch: async (input, init) => {
      ...
      const useMain =
        ...
        host === "file.tapd.cn" ||
        host === "www.tapd.cn" ||
        host.endsWith(".tapd.cn")
      if (!useMain) return fetch(input, init)
      ...
      if (init?.body != null && typeof init.body !== "undefined") {
        ...
        if (typeof init.body === "string") {
          ...
          bodyBase64 = encodeBytes(new Uint8Array(init.body))
        } else if (ArrayBuffer.isView(init.body)) {
          bodyBase64 = encodeBytes(new Uint8Array(init.body.buffer, init.body.byteOffset, init.body.byteLength))
        } else {
          // FormData / ReadableStream — fall back to renderer fetch
          return fetch(input, init)
        }
      }

      const result = await window.api.fetchExternal({
        url,
        method: init?.method,
        headers,
        bodyBase64,
