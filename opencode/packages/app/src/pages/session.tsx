...
export default function Page() {
  ...
  const desktopReviewColumnOpen = createMemo(() =>
    newSessionDesign() ? desktopV2ReviewOpen() : desktopReviewOpen(),
  )
  const fileTreeLeft = createMemo(
    () => desktopFileTreeOpen() && layout.fileTree.position() === "left",
  )
  const fileTreeRight = createMemo(
    () => desktopFileTreeOpen() && layout.fileTree.position() === "right",
  )
  const chatOnLeft = createMemo(() => layout.session.chatPosition() === "left")
  const chatResizeEdge = createMemo((): "start" | "end" => (chatOnLeft() ? "end" : "start"))
  ...
  // When the session file tree is open as a chat sibling, reserve its width in
  // the chat clamp so chat+review still fit beside it on either side.
  const sessionFileTreeReserve = createMemo(() =>
  ...
  const sessionPanelAvailable = createMemo(() => {
    ...
    const treeGaps = sessionFileTreeReserve() > 0 ? gap : 0
    return width - gap - treeGaps
  })
  const sessionPanelMax = createMemo(() => {
    const available = sessionPanelAvailable()
    if (available === undefined) return 1000
    return sessionPanelWidthMax({
      available,
      split: splitReview(),
      fileTree: sessionFileTreeReserve(),
    })
  })
  ...
  const sessionPanelWidth = createMemo(() => {
    if (!desktopSessionResizeOpen() && !desktopFileTreeOpen()) return "100%"
  ...
  // Cursor-style: one primary explorer. When the session file tree is shown,
  // collapse the nested review file browser so the two trees do not stack.
  // Wait for persist hydration so a stored sidebarOpened:true cannot reopen it.
  createEffect(() => {
    if (!reviewV2State.sidebarTransition()) return
    if (desktopFileTreeOpen()) reviewV2State.closeSidebar()
  })
      ...
      >
        ...
        <Show when={fileTreeLeft()}>
          <SessionFileTreePanel
            ...
            reviewOpen={desktopReviewColumnOpen}
        ...
        <div
          classList={{
            ...
            "md:order-2": chatOnLeft(),
            "md:order-3": !chatOnLeft(),
          }}
          style={{
          ...
          <Show when={desktopSessionResizeOpen()}>
            <div onPointerDown={() => size.start()}>
              <ResizeHandle
                edge={chatResizeEdge()}
                classList={{
                  "-right-1": settings.general.newLayoutDesigns() && chatOnLeft(),
                  "-left-1": settings.general.newLayoutDesigns() && !chatOnLeft(),
                }}
        ...
        <Show when={!newSessionDesign() && desktopReviewOpen()}>
          <SessionSidePanel
            class={chatOnLeft() ? "md:order-3" : "md:order-2"}
            canReview={canReview}
            diffs={reviewDiffs}
            diffsReady={reviewReady}
        ...
        <Show when={newSessionDesign()}>
          <Show when={isDesktop() ? desktopV2PanelLayout().visible : terminalOpen()}>
            <div
              classList={{
                "min-w-0 h-full flex flex-1 flex-col": true,
                "md:order-3": chatOnLeft(),
                "md:order-2": !chatOnLeft(),
              }}
            >
              <Show when={isDesktop() && desktopV2ReviewOpen()}>
