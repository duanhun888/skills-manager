...
describe("sessionPanelWidthMax", () => {
  ...
  test("reserves sibling file tree width on either side of chat", () => {
    expect(sessionPanelWidthMax({ available: 1700, split: false, fileTree: 240 })).toBe(
      1700 - REVIEW_PANE_WIDTH_MIN - 240,
    )
    expect(sessionPanelWidthMax({ available: 900, split: false, fileTree: 240 })).toBe(SESSION_PANEL_WIDTH_MIN)
  })
})
...
describe("clampSessionPanelWidth", () => {
  ...
  test("accounts for a left or right file tree sibling", () => {
    expect(clampSessionPanelWidth({ width: 1600, available: 1700, split: false, fileTree: 240 })).toBe(
      1700 - REVIEW_PANE_WIDTH_MIN - 240,
    )
  })
})
