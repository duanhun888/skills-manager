      ...
      >
        <div class="mx-auto grid min-h-full w-full max-w-[1080px] grid-rows-[auto_minmax(0,1fr)_auto] gap-4 px-3 lg:grid-cols-[280px_minmax(0,720px)] lg:grid-rows-1 lg:gap-8 lg:px-6">
          <HomeProjectColumn
            ...
            openRequirements={() => navigate("/requirements")}
            language={language}
            onWheel={(event) => {
  ...
  return (
    ...
    >
      ...
      <HomeUtilityNav
        ...
        openRequirements={props.openRequirements}
        language={props.language}
      />
    </aside>
  )
  ...
  return (
    <div class={`${props.class ?? ""} min-w-0 flex-col gap-1 pr-3`}>
      <button
        ...
        onClick={props.openRequirements}
      >
        <IconV2 name="edit" size="small" />
        <span class={HOME_PROJECT_NAV_LABEL}>{props.language.t("sidebar.nav.requirements")}</span>
      </button>
      <button
        type="button"
        class={`${HOME_PROJECT_NAV_ROW} text-v2-text-text-faint [&>[data-slot=icon-svg]]:text-v2-icon-icon-muted`}
