          ...
          >
            ...
            >
              ...
              >
                <Button
                  variant="ghost"
                  data-action="titlebar-settings"
                  class="titlebar-icon h-7 shrink-0 gap-1.5 rounded-md px-2"
                  onClick={() => command.trigger("settings.open")}
                  aria-label={language.t("command.settings.open")}
                >
                  <Icon name="settings-gear" size="small" />
                  <span class="text-12-medium">{language.t("sidebar.settings")}</span>
                </Button>
              </TooltipKeybind>
              <Show when={windows()}>
...
function TitlebarV2Right(props: { state: TitlebarV2RightState }) {
  ...
  const workbench = createMemo<"home" | "requirements" | "coding">(() => {
  ...
  const goHome = () => {
    if (workbench() === "home") return
    navigate("/")
  }

  const goRequirements = () => {
    if (workbench() === "requirements") return
    navigate("/requirements")
  }

  const goCoding = () => {
    if (workbench() === "coding") return
    if (settings.general.newLayoutDesigns()) {
      const tab = tabs.store[tabs.store.length - 1]
      if (tab) {
        tabs.select(tab)
        return
      }
      const directory = server.projects.last() ?? layout.projects.list()[0]?.worktree
      if (directory) {
  ...
  const segmentClass = (id: "home" | "requirements" | "coding") => ({
    "text-v2-text-text-base after:opacity-100": workbench() === id,
    "text-v2-text-text-weak hover:text-v2-text-text-base after:opacity-0 hover:after:opacity-40": workbench() !== id,
  })

  return (
    <div class="relative z-20 flex shrink-0 items-center justify-end gap-0.5 overflow-visible">
      <Show when={props.state.update.visible}>
        <TitlebarUpdateIconButton state={props.state.update} />
      </Show>
      <div id="opencode-titlebar-right" class="flex shrink-0 items-center justify-end gap-0" />
      <div
        data-component="titlebar-workbench-switch"
        class="mr-1 inline-flex h-7 items-center gap-0.5"
        role="tablist"
        aria-label={language.t("workbench.switch")}
      >
        <TooltipV2 placement="bottom" value={language.t("workbench.home")}>
          <button
            type="button"
            role="tab"
            data-action="titlebar-workbench-home"
            aria-selected={workbench() === "home"}
            class="relative inline-flex h-7 items-center gap-1 rounded-md px-2 text-12-medium transition-colors after:absolute after:inset-x-2 after:bottom-0.5 after:h-px after:rounded-full after:bg-current after:transition-opacity"
            classList={segmentClass("home")}
            onClick={goHome}
          >
            <span>{language.t("workbench.home")}</span>
          </button>
        </TooltipV2>
        <TooltipV2 placement="bottom" value={language.t("workbench.requirements")}>
          <button
            type="button"
            role="tab"
            data-action="titlebar-workbench-requirements"
            aria-selected={workbench() === "requirements"}
            class="relative inline-flex h-7 items-center gap-1 rounded-md px-2 text-12-medium transition-colors after:absolute after:inset-x-2 after:bottom-0.5 after:h-px after:rounded-full after:bg-current after:transition-opacity"
            classList={segmentClass("requirements")}
            onClick={goRequirements}
          >
            <span>{language.t("workbench.requirements")}</span>
          </button>
        </TooltipV2>
        <TooltipV2 placement="bottom" value={language.t("workbench.coding")}>
          <button
            type="button"
            role="tab"
            data-action="titlebar-workbench-coding"
            aria-selected={workbench() === "coding"}
            class="relative inline-flex h-7 items-center gap-1 rounded-md px-2 text-12-medium transition-colors after:absolute after:inset-x-2 after:bottom-0.5 after:h-px after:rounded-full after:bg-current after:transition-opacity"
            classList={segmentClass("coding")}
            onClick={goCoding}
          >
            <span>{language.t("workbench.coding")}</span>
          </button>
        </TooltipV2>
      </div>
      <TooltipV2
        ...
        value={
          <>
            ...
            <Show when={command.keybindParts("settings.open").length > 0}>
      ...
      >
        <button
          type="button"
          data-action="titlebar-settings"
          class="inline-flex h-7 shrink-0 items-center gap-1.5 rounded-md px-2 text-12-medium text-v2-text-text-weak hover:bg-v2-overlay-simple-overlay-hover hover:text-v2-text-text-base"
          onClick={() => command.trigger("settings.open")}
          aria-label={language.t("command.settings.open")}
        >
