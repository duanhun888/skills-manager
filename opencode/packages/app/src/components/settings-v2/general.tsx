...
import { useLayout, type ChatPosition, type FileTreePosition } from "@/context/layout"
...
export const SettingsGeneralV2: Component<{
  ...
  const fileTreePositionOptions = createMemo((): { value: FileTreePosition; label: string }[] => [
    { value: "left", label: language.t("settings.general.row.fileTreePosition.option.left") },
    { value: "right", label: language.t("settings.general.row.fileTreePosition.option.right") },
  ])
  const chatPositionOptions = createMemo((): { value: ChatPosition; label: string }[] => [
    { value: "left", label: language.t("settings.general.row.chatPosition.option.left") },
    { value: "right", label: language.t("settings.general.row.chatPosition.option.right") },
  ])
    ...
    <div class="settings-v2-section">
      ...
      <SettingsListV2>
        ...
        >
          <SelectV2
            appearance="inline"
            data-action="settings-file-tree-position"
            options={fileTreePositionOptions()}
            placement="bottom-end"
            gutter={6}
            current={fileTreePositionOptions().find((o) => o.value === layout.fileTree.position())}
            value={(o) => o.value}
            label={(o) => o.label}
            disabled={!settings.general.showFileTree()}
            onSelect={(option) => {
              if (!option) return
              layout.fileTree.setPosition(option.value)
            }}
          />
        </SettingsRowV2>

        <SettingsRowV2
          title={language.t("settings.general.row.chatPosition.title")}
          description={language.t("settings.general.row.chatPosition.description")}
        >
          <SelectV2
            appearance="inline"
            data-action="settings-chat-position"
            options={chatPositionOptions()}
            placement="bottom-end"
            gutter={6}
            current={chatPositionOptions().find((o) => o.value === layout.session.chatPosition())}
            value={(o) => o.value}
            label={(o) => o.label}
            onSelect={(option) => {
              if (!option) return
              layout.session.setChatPosition(option.value)
            }}
          />
        </SettingsRowV2>
