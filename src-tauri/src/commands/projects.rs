...
fn project_to_dto(
    ...
    ProjectDto {
        ...
        server_project_id: rec.server_project_id.clone(),
    }
}
...
pub async fn import_project_skill_to_center(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let skill_record = SkillRecord {
            ...
            scope: crate::core::skill_store::SKILL_SCOPE_PROJECT.to_string(),
...
#[tauri::command]
pub async fn project_agent_has_skill_dir(
    store: State<'_, Arc<SkillStore>>,
    project_id: String,
    agent: String,
    dir_name: String,
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let (skills_root, disabled_root) =
            resolve_agent_skills_roots(&store, &project, &agent)
                .ok_or_else(|| AppError::not_found(format!("Unknown agent: {agent}")))?;
        let enabled_path = skills_root.join(&dir_name);
        let disabled_path = disabled_root.as_ref().map(|root| root.join(&dir_name));
        Ok(enabled_path.is_dir() || disabled_path.is_some_and(|p| p.is_dir()))
    })
    .await
    .map_err(|e| AppError::internal(format!("dir check failed: {e}")))?
}
                    ...
                    .unwrap_or(false)
            {
                return Err(AppError::invalid_input(format!(
                    "Skill \"{}\" already exists in this workspace for agent {}. \
                     Remove the existing folder in the project workspace or rename the skill.",
...
#[tauri::command]
pub async fn set_project_server_link(
    project_id: String,
    ...
    tauri::async_runtime::spawn_blocking(move || {
        store
            .set_project_server_id(&project_id, server_project_id.as_deref())
            .map_err(AppError::db)?;
        let rec = store
            .get_project_by_id(&project_id)
            .map_err(AppError::db)?
        ...
        let all_managed = store.get_all_skills().map_err(AppError::db)?;
        let configs = agent_skill_configs(&store);
        Ok(project_to_dto(&rec, &all_managed, &configs))
    })
    .await?
}
