...
pub struct ManagedSkillDto {
    ...
    /// Semver from SKILL.md frontmatter (`version` or `metadata.version`).
...
pub(crate) fn log_install_outcome(
    store: &SkillStore,
    source_label: &str,
...
pub(crate) fn log_update_outcome(
    store: &SkillStore,
    skill_id: &str,
    source_label: &str,
...
fn managed_skill_to_dto(
    ...
    if meta.category.as_deref() != skill.category.as_deref() {
        let _ = store.update_skill_category(&skill.id, meta.category.as_deref());
    }
    let description = meta
        .description
        .filter(|s| !s.trim().is_empty())
        .or(skill.description);
    let category = meta.category.or(skill.category.clone());
    let mut tags = tags_map.get(&skill.id).cloned().unwrap_or_default();
    ...
    ManagedSkillDto {
        ...
        tags,
        scope: skill.scope,
        server_skill_id: skill.server_skill_id,
...
pub fn reimport_local_skill_internal(
    ...
    let result = (|| -> Result<(), AppError> {
        ...
        store
            ...
            .map_err(AppError::db)?;
        resync_copy_targets(store, &skill.id)?;
        let _ = skill_metadata::persist_metadata_from_disk(
            store,
            &skill.id,
...
pub fn store_installed_skill_unlocked(
    ...
    let record = SkillRecord {
        ...
        scope: crate::core::skill_store::default_skill_scope(),
        server_skill_id: None,
        category: None,
    };

    store.insert_skill(&record).map_err(AppError::db)?;
    let central_for_meta = record.central_path.clone();
    let _ = skill_metadata::persist_metadata_from_disk(store, &id, Path::new(&central_for_meta));
    if let Some(scenario_id) = active_scenario_id {
        store
...
pub async fn set_skill_category(
    ...
    if category.as_deref().is_some_and(|c| !c.trim().is_empty()) && normalized.is_none() {
        return Err(AppError::invalid_input("invalid skill category"));
    }
    let store = store.inner().clone();
    tauri::async_runtime::spawn_blocking(move || {
        sync_metadata::with_repo_lock("set skill category", || {
            let skill = store
                .get_skill_by_id(&skill_id)?
                .ok_or_else(|| anyhow::anyhow!("Skill not found"))?;
            store.update_skill_category(&skill_id, normalized.as_deref())?;
            skill_metadata::write_category_to_skill_md(
                Path::new(&skill.central_path),
                normalized.as_deref(),
            )?;
            if let Ok(hash) =
            ...
            resync_copy_targets(&store, &skill_id)
...
pub async fn set_skill_scope(
    ...
    let store = store.inner().clone();
    tauri::async_runtime::spawn_blocking(move || {
        store.update_skill_scope(&skill_id, &scope).map_err(AppError::db)?;
        managed_skill_by_id(&store, &skill_id)
    })
    .await?
}
...
#[tauri::command]
pub async fn set_server_skill_id(
    skill_id: String,
    server_skill_id: Option<String>,
    store: State<'_, Arc<SkillStore>>,
) -> Result<(), AppError> {
    let store = store.inner().clone();
    tauri::async_runtime::spawn_blocking(move || {
        store
            .set_server_skill_id(&skill_id, server_skill_id.as_deref())
            .map_err(AppError::db)
    })
    .await?
}
...
#[tauri::command]
pub async fn get_linked_server_project_id(
    store: State<'_, Arc<SkillStore>>,
) -> Result<Option<String>, AppError> {
    let store = store.inner().clone();
    tauri::async_runtime::spawn_blocking(move || {
        store.first_linked_server_project_id().map_err(AppError::db)
    })
    .await?
}
