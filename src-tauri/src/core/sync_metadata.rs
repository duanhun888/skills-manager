...
pub(crate) fn reindex_from_metadata_unlocked(store: &SkillStore) -> Result<()> {
    ...
    for meta in skills {
        ...
        let record = SkillRecord {
            ...
            scope: previous
                .map(|s| s.scope.clone())
                .unwrap_or_else(crate::core::skill_store::default_skill_scope),
            server_skill_id: previous.and_then(|s| s.server_skill_id.clone()),
            category: parsed.category.clone().or_else(|| previous.and_then(|s| s.category.clone())),
        };
