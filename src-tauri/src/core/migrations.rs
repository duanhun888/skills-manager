...
/// v6 → v7: Add `scope` to `skills` for personal / org / project classification.
...
/// v7 → v8: Link local skills/projects to central server registry IDs.
fn migrate_v7_to_v8(conn: &Connection) -> Result<()> {
    add_column_if_missing(conn, "skills", "server_skill_id", "TEXT")?;
    add_column_if_missing(conn, "projects", "server_project_id", "TEXT")?;
    Ok(())
}

/// v8 → v9: Business category on skills (SKILL.md `category` field).
fn migrate_v8_to_v9(conn: &Connection) -> Result<()> {
    add_column_if_missing(conn, "skills", "category", "TEXT")?;
    Ok(())
}
