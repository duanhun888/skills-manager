...
pub const SKILL_SCOPE_PERSONAL: &str = "personal";
pub const SKILL_SCOPE_ORG: &str = "org";
pub const SKILL_SCOPE_PROJECT: &str = "project";

pub fn default_skill_scope() -> String {
    SKILL_SCOPE_PERSONAL.to_string()
}

pub fn normalize_skill_scope(scope: &str) -> anyhow::Result<String> {
    match scope {
        SKILL_SCOPE_PERSONAL | SKILL_SCOPE_ORG | SKILL_SCOPE_PROJECT => Ok(scope.to_string()),
        _ => anyhow::bail!("invalid skill scope: {scope}"),
    }
}
...
pub struct SkillRecord {
    ...
    pub scope: String,
    /// Central server registry skill id (MySQL), when published.
    pub server_skill_id: Option<String>,
    /// Business category id (see skill_categories).
...
impl Default for SkillRecord {
    fn default() -> Self {
        Self {
            ...
            scope: default_skill_scope(),
            server_skill_id: None,
            category: None,
        }
    }
}
...
pub struct ProjectRecord {
    ...
    pub created_at: i64,
    pub updated_at: i64,
    /// Central server project id when linked for project-scoped skills.
...
impl SkillStore {
    ...
    pub fn insert_skill(&self, skill: &SkillRecord) -> Result<()> {
        ...
        conn.execute(
            "INSERT INTO skills (
                ...
                created_at, updated_at, status, update_status, last_checked_at, last_check_error, scope,
                server_skill_id, category
             )
    ...
    pub fn get_skill_by_server_skill_id(
        &self,
        server_skill_id: &str,
        ...
        let mut stmt = conn.prepare(
            "SELECT id, name, description, source_type, source_ref, source_ref_resolved, source_subpath,
                    ...
                    created_at, updated_at, status, update_status, last_checked_at, last_check_error, scope,
                    server_skill_id, category
             FROM skills WHERE server_skill_id = ?1",
        )?;
        let mut rows = stmt.query_map(params![server_skill_id], map_skill_row)?;
        Ok(rows.next().and_then(|r| r.ok()))
    }
    ...
    pub fn update_skill_scope(&self, id: &str, scope: &str) -> Result<()> {
        let scope = normalize_skill_scope(scope)?;
        let conn = self.conn.lock().unwrap();
        let now = chrono::Utc::now().timestamp_millis();
        conn.execute(
            "UPDATE skills SET scope = ?1, updated_at = ?2 WHERE id = ?3",
            params![scope, now, id],
        )?;
        Ok(())
    }

    pub fn set_server_skill_id(&self, id: &str, server_skill_id: Option<&str>) -> Result<()> {
        let conn = self.conn.lock().unwrap();
        let now = chrono::Utc::now().timestamp_millis();
        conn.execute(
            "UPDATE skills SET server_skill_id = ?1, updated_at = ?2 WHERE id = ?3",
            params![server_skill_id, now, id],
        )?;
        Ok(())
    }

    pub fn first_linked_server_project_id(&self) -> Result<Option<String>> {
        let conn = self.conn.lock().unwrap();
        let mut stmt = conn.prepare(
            "SELECT server_project_id FROM projects
             WHERE server_project_id IS NOT NULL AND server_project_id != ''
    ...
    pub fn get_skills_for_scenario(&self, scenario_id: &str) -> Result<Vec<SkillRecord>> {
        ...
        let mut stmt = conn.prepare(
            "SELECT s.id, s.name, s.description, s.source_type, s.source_ref, s.source_ref_resolved, s.source_subpath,
                    ...
                    s.created_at, s.updated_at, s.status, s.update_status, s.last_checked_at, s.last_check_error, s.scope,
                    s.server_skill_id, s.category
             FROM skills s
    ...
    pub fn insert_project(&self, project: &ProjectRecord) -> Result<()> {
        ...
        conn.execute(
            "INSERT INTO projects (
                ...
                sort_order, created_at, updated_at, server_project_id
             )
    ...
    pub fn get_all_projects(&self) -> Result<Vec<ProjectRecord>> {
        ...
        let rows = stmt.query_map([], |row| {
            Ok(ProjectRecord {
                ...
                server_project_id: row.get(10).ok(),
            })
        })?;
        Ok(rows.filter_map(|r| r.ok()).collect())
    }
    ...
    pub fn set_project_server_id(&self, id: &str, server_project_id: Option<&str>) -> Result<()> {
        let conn = self.conn.lock().unwrap();
        let now = chrono::Utc::now().timestamp_millis();
        conn.execute(
            "UPDATE projects SET server_project_id = ?1, updated_at = ?2 WHERE id = ?3",
            params![server_project_id, now, id],
        )?;
        Ok(())
    }
    ...
    pub fn update_skill_content_hash(&self, id: &str, content_hash: &str) -> Result<()> {
        let conn = self.conn.lock().unwrap();
        let now = chrono::Utc::now().timestamp_millis();
        conn.execute(
...
fn map_skill_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<SkillRecord> {
    Ok(SkillRecord {
        ...
        scope: row.get(19).unwrap_or_else(|_| default_skill_scope()),
        server_skill_id: row.get(20).ok(),
        category: row.get(21).ok(),
    })
}
